...
var authentication = {
    ...
    GetUsersArray: function (users_url) {
...
var usersArray = JSON.parse(authentication.GetUsersArray("https://studentschat.herokuapp.com/users/"));
...
var messagesData = JSON.parse(authentication.getMessages());
...
signButton.onclick = function () {
    ...
    var currTime = new Date(messagesData[messagesData.length-1].datetime);

    var lastMsg = {
        lastMsgMin: currTime.getMinutes(),
        lastMsgSec: currTime.getSeconds()
    };

    function creatingDomMessages() {
        var total_msg, user_info, dialog, msgData, textInfo, userFirstName,  msgDateHour, msgMinutes, msgTime, currentTime,
            currentDate;


        console.log(lastMsg.lastMsgMin+" "+lastMsg.lastMsgSec);

        (function drawingMsgs () {
    ...
    creatingDomMessages();
        ...
        function getNewMsgs () {
            var checkingNewMessages = setInterval(function() {
                var messagesData = JSON.parse(authentication.getMessages());
                var currTime = new Date(messagesData[messagesData.length-1].datetime);
                var lastMsgNew = {
                    lastMsgMin: currTime.getMinutes(),
                ...
                if((lastMsg.lastMsgMin && lastMsg.lastMsgSec)!==(lastMsgNew.lastMsgMin && lastMsgNew.lastMsgSec)) {

                    /*Какое условие проверки мне надо сюда написать?*/
